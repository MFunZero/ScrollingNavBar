...
class RefreshHeaderView: RefreshBaseView {
    ...
    //refreshNow
    class func refreshNow()->RefreshHeaderView{
        let refreshNow:RefreshHeaderView  = RefreshHeaderView(frame: CGRectMake(0, 0,   UIScreen.mainScreen().bounds.width,  CGFloat(RefreshViewHeight)))
        return refreshNow
    }
    // 最后的更新时间
