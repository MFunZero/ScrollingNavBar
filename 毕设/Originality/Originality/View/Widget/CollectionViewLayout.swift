...
class CollectionViewLayout:  UICollectionViewLayout{
    ...
    */
    override func collectionViewContentSize() -> CGSize {
        let height = ((self.collectionView!.bounds.width) / CGFloat(countInLine) ) * 1.8 * CGFloat(self.collectionView!.numberOfItemsInSection(0) / countInLine) + 280
...
//        
    ...
    override func layoutAttributesForItemAtIndexPath(indexPath: NSIndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        //当前行y坐标
        let lineOriginY = (smallCellSide) * CGFloat(lines) + top + 280 
...
//    
        override func layoutAttributesForSupplementaryViewOfKind(elementKind: String, atIndexPath indexPath: NSIndexPath) -> UICollectionViewLayoutAttributes? {
            ...
            attribute.frame = CGRect(x: 0,y: 0 ,width: self.collectionView!.frame.width,height: 280)
           
            }else {
                let height = ((self.collectionView!.bounds.width) / CGFloat(countInLine) ) * 1.8 * CGFloat(self.collectionView!.numberOfItemsInSection(0) / countInLine) + 450
